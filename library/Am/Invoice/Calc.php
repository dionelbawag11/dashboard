...
class Am_Invoice_Calc_Tax_Absorb extends Am_Invoice_Calc_Tax
{
    ...
    public function calculatePiece(stdClass $fields)
    {
        ...
        if ($fields->tax_group && ($fields->tax_group != IProduct::NO_TAX))
        {
            ...
            $this->item->data()->set("orig_{$this->currentPrefix}shipping", $shipping);
            if ($this->getPlugin()->getConfig('shipping')) {
                $this->invoiceBill->data()->set('shipping_taxable', 1);
                $fields->shipping = Am_Currency::moneyRound($fields->shipping / (1 + $tax_rate/100), $currency);
            }

            $price_precise = $price * 100 / ($tax_rate + 100);

            $fields->price = round($price_precise * 100)/100;
            ...
            $fields->tax = $fields->tax_rate ? ($price * $fields->qty + $shipping - $discount) - ($fields->price * $fields->qty + $fields->shipping - $fields->discount) : 0;
        } else {
            $fields->tax = 0.0;
        }
    }
...
class Am_Invoice_Calc_Zero extends Am_Invoice_Calc
{
    public function calculatePiece(stdClass $fields)
    {
        ...
        $fields->price = $orig_price ?: $fields->price;
        ...
        $this->invoiceBill->data()->set('shipping_taxable', 0);
    }
}
...
class Am_Invoice_Calc_Tax_ChangeRate extends Am_Invoice_Calc_Tax_Absorb
{
    ...
    {
        $fields->price = ($fields->total+$fields->discount)/$fields->qty;
