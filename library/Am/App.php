...
interface Am_Locker_Interface
{
    function lock(int $seconds);
    function unlock();
}

class Am_Exception_CannotAcquireLock extends Am_Exception {}

class Am_Locker_Db implements Am_Locker_Interface
{
    private $db;
    protected $id;
    protected $locked = false;

    function __construct(string $id, Am_Db $db)
    {
        $this->id = $id;
        $this->db = $db;
    }
    ...
    function lock(int $seconds)
    {
        ...
        if (!$this->db->selectCell("SELECT GET_LOCK(?, 0)", $this->id))
            throw new Am_Exception_CannotAcquireLock("Cannot get lock");
        $this->locked = true;
    }

    function unlock()
    {
        $this->db->query("SELECT RELEASE_LOCK(?)", $this->id);
    }
}

class Am_Locker_Cache implements Am_Locker_Interface
{
    protected $id;
    protected $cache;

    function __construct(string $id, Zend_Cache_Core $cache)
    {
        $this->id = 'locker_'.$id;
        $this->cache = $cache;
    }
    ...
    function lock(int $seconds)
    {
        $this->cache->save(1, $this->id, [], $seconds);
    }
...
class Am_Locker_Null implements Am_Locker_Interface
{
    function lock(int $seconds) {}
    function unlock() {}
}
...
class Am_Cron
{
    const FIRST_RUN = 'cron-first-run';
    ...
    {
        $this->di = $di;
        $this->lockId = 'am-cron-lock-'.md5(__FILE__);
        $this->locker = new Am_Locker_Null;
        $this->events = [
            Am_Event::DAILY => 'dmy', // this one must be run first to recalc user status
            Am_Event::HOURLY => 'dmyH',
            Am_Event::EVERY_5MIN => 'dmyH', // additional code checks required
            Am_Event::WEEKLY => 'Wy',
            Am_Event::MONTHLY => 'my',
            Am_Event::YEARLY => 'y',
        ];
    ...
    public function addCriticalTask($functionSignature)
    {
        $this->critical[$functionSignature] = true;
    }
    ...
    public function needRun()
    {
        ...
        if ($ret === false)
        {
            $cacheKey = 'cron_need_run_' . md5(__FILE__);
            $ret = $this->di->cache->load($cacheKey);
            if ($ret === false) {
                $tt = $this->tasksToRun($this->di->time);
                $ret = 0;
                array_walk_recursive($tt, function($v) use (&$ret) { $ret++; } );
                $this->di->cache->save($ret, $cacheKey, [], $ret ? 2 : 30);
            }
        }
        return (bool)$ret;
    }
    ...
    {
        $r = $this->di->store->get(self::FIRST_RUN);
        if ($r<=0)
            $this->di->store->set(self::FIRST_RUN, $r = $this->di->time);
        return (int)$r;
    }

    /**
     * @access private
     * @return array event => HookCallback[]
     */
    public function tasksToRun($time)
    {
        $ret = [];
        $db = $this->di->db;
        ...
        try
        {
            ...
            SELECT c1.event, c1.callback, c1.* FROM ?_cron c1
            INNER JOIN 
            (SELECT max(tm_started) AS tm_started, event, callback
            FROM ?_cron
            GROUP BY event, callback) c2
            ON (c1.event = c2.event AND c1.callback = c2.callback AND c1.tm_started = c2.tm_started)            
SQL
            ) as $row)
                $rows[$row['event']][$row['callback']] = $row;
        } catch (\Exception $e) {
            if ($e->getCode() == 1146) // table does not exists, skip error to allow admin-upgrade-db
                return [];
            else
                throw $e;
        }
        ...
        $twoDaysBefore = $time - 24 * 3600 * 2;
        foreach ($this->events as $evt => $testString)
        {
            $callbacks = $this->di->hook->getRegisteredHooks($evt);
            /** @var Am_HookCallback[] $callbacks */
            foreach ($callbacks as $kk => $callback)
            {
                $row = $rows[$evt][$callback->getSignature()] ?? null;
                // check if this task has not been running same hour/day/month
                if ($evt == Am_Event::HOURLY || $evt == Am_Event::DAILY)
                {
                    $tm_started = ($row && $row['tm_started']) ? $row['tm_started'] : $twoDaysBefore;
                } else {
                    $tm_started = ($row && $row['tm_started']) ? $row['tm_started'] : $firstRun;
                }
                $shouldAdd = date($testString, (int)$tm_started)!=date($testString, $time);
                if ($evt == Am_Event::EVERY_5MIN)
                {
                    $round5 = function($tm) {
                        return intval(date('i', (int)$tm)/5);
                    };
                    $shouldAdd = $shouldAdd || ($round5($tm_started) != $round5($time));
                }
                if ($shouldAdd)
                {
                    $ret[$evt][] = $callback;
    ...
    /** @access private */
    public function tasksRun(array $tasks, $log = null, $tm_cron = null)
    {
        if ($log === null)
            $log = function(){};
        $di = $this->di;
        if ($tm_cron === null)
            $tm_cron = $di->time;
        foreach ($tasks as $evt => $callbacks)
        {
            $event = new Am_Event($evt, [
                'date' => sqlDate('now'),
                'datetime' => sqlTime('now')
            ]);
            /** @var Am_HookCallback $h */
            foreach ($callbacks as $h)
            {
                $tm = $this->di->microtime;
                if ($h->getFile())
                    include_once $di->root_dir . DIRECTORY_SEPARATOR . $h->getFile();
                try {
                    $this->locker->lock(600); // update lock
                    $tm = $di->microtime;
                    $di->db->query("INSERT INTO ?_cron SET tm_started=?,tm_cron=?d,event=?,callback=?,flags=?d",
                        $tm, $tm_cron, $evt, $h->getSignature(), $this->taskFlags($h));
                    $dbId = $di->db->selectCell("SELECT LAST_INSERT_ID()");

                    $event->call($h);

                    $tm = $di->microtime - $tm;
                    $log("run [$evt] callback " . $h->getSignature() . sprintf(' %.2f', $tm*1000) . " ms");

                    if ($event->mustStop())
                    {
                        $di->db->query("UPDATE ?_cron SET tm_finished=?, error=? WHERE cron_id=?d",
                            $di->mcrotime, json_encode(['message' => 'event stopped by this callback']), $dbId);
                        break;
                    } else {
                        $di->db->query("UPDATE ?_cron SET tm_finished=? WHERE cron_id=?d",
                            $di->microtime, $dbId);
                    }
                } catch (\Throwable $e) {
                    $critical = $this->isCriticalTask($h);
                    $di->db->query("UPDATE ?_cron SET tm_finished=?, error=? WHERE cron_id=?d",
                        $di->microtime, json_encode(['exception'=>get_class($e), 'message'=>$e->getMessage(),'trace'=>$e->getTraceAsString()]), $dbId);
                    if ($critical)
                    {
                        $msg = "Cron [$evt] critical task failed [".$h->getSignature()."]";
                        $log($msg);
                        $this->di->logger->error($msg);

                        return; // critical task is failed, game over

                    } else {
                        $msg = "Cron [$evt] task failed [".$h->getSignature()."]";
                        $log($msg);
                    }
                }
            }
    ...
    /**
     * Critical task logic is not finished yet
     * DO NOT RELY ON IT
     *
     * @param Am_HookCallback $callback
     * @return bool
     */
    public function isCriticalTask(Am_HookCallback $callback)
    {
        return !empty($this->critical[$callback->getSignature()]);
    }

    public function taskFlags(Am_HookCallback $callback)
    {
        return $this->isCriticalTask($callback) ? self::FLAG_CRITICAL : 0;
    }
    ...
    public function checkCron($log = null)
    {
        ...
        $this->di->plugins_payment->loadEnabled()->getAllEnabled();
        try {
            $tasks = $this->tasksToRun($this->di->time);
            if ($tasks) {
                $this->setLimits();
                $log("cron started");
                $this->tasksRun($tasks, $log);
                $log('cron finished', array_keys($tasks));
            } else {
                $log("cron - no tasks to run");
            }
        } finally {
            $this->locker->unlock();
        }
    ...
    {
        return $this->di->db->selectCell("SELECT tm_cron FROM ?_cron ORDER BY cron_id DESC LIMIT 1") ?:
            $this->di->db->selectCell("SELECT `value` FROM ?_store WHERE name=?", 'cron-last-run');
    }
    ...
    {
        $this->di->db->query("DELETE FROM ?_cron WHERE tm_cron<?d AND event NOT IN (?a)",
            $this->di->time - 3600*24*7,
            [Am_Event::YEARLY, Am_Event::MONTHLY]
        );
    }
...
class Am_Laz  {
    protected $diObj, $method;
    /**
     * This function intended to avoid immedate object initialization in Di
     * So insted of writing
     *     $di->hook->add(Am_Event::HOURLY, [$di->mailQueue, 'sendFromQueue']);
     * you use
     *     $di->hook->add(Am_Event::HOURLY, Am_Laz::y('mailQueue', 'sendFromQueue'));
     *
     * in this case object will be created only when callback is called
     */
    static function y($diObj, $method) : self {
        $x = new self;
        $x->diObj = $diObj;
        $x->method = $method;
        return $x;
    ...
    public function getSignature() { return sprintf('di->%s->%s', $this->diObj, $this->method); }

    public function __invoke()
    {
        return call_user_func_array([Am_Di::getInstance()->{$this->diObj}, $this->method], func_get_args());
    }
}
...
class Am_App
{
    ...
    public function bootstrap()
    {
        ...
        // Load user in order to check maybe we need to refresh user's session;
...
//            Zend_Translate::setCache($cache);
    ...
    public function initHooks()
    {
        ...
        $di->plugins_storage->loadEnabled()->getAllEnabled();

        $di->plugins_tax->getAllEnabled();


        $hook = $di->hook;
    
        $hook->add(Am_Event::HOURLY, Am_Laz::y('emailTemplateTable', 'sendCronHourlyPendingNotifications'));
        $hook->add(Am_Event::EVERY_5MIN, Am_Laz::y('otpTable', 'cleanUp'));
        $hook->add(Am_Event::EVERY_5MIN, Am_Laz::y('mailQueue',  'sendFromQueue'));
        $hook->add(Am_Event::EVERY_5MIN, Am_Laz::y('smsQueueTable',  'process'));
        $hook->add(Am_Event::HOURLY, [Am_Session_SaveHandler::class, 'cleanUp']);

        /* $hook->add(Am_Event::DAILY, [$di->app,  'onDaily')); // <- split by calls below */
        $hook->add(Am_Event::DAILY, Am_Laz::y('userTable',  'checkAllSubscriptions'));
        $di->cron->addCriticalTask('di->userTable->checkAllSubscriptions');
        $di->emailTemplateTable->setupHooks();

        $hook->add(Am_Event::DAILY, Am_Laz::y('store',  'cronDeleteExpired'));
        $hook->add(Am_Event::DAILY, Am_Laz::y('storeRebuild',  'cronDeleteExpired'));
        $hook->add(Am_Event::DAILY, [Am_Auth_BruteforceProtector::class, 'cleanUp']);
        $hook->add(Am_Event::DAILY, [$this, 'cleanupTables']);

        $di->savedReportTable->setupHooks();

        if (!$di->config->get('use_cron'))
            $di->cron->setupHook();
    ...
    /**
     * Remove old/expire records from tables
     * used by cron
     */
    public function cleanupTables()
    {
        ...
        if ($di->config->get('clear_access_log') && $di->config->get('clear_access_log_days') > 0) {
            $dat = sqlDate($di->time - $di->config->get('clear_access_log_days') * 3600 * 24);
            $di->accessLogTable->clearOld($dat);
        }

        if ($di->config->get('clear_debug_log_days') > 0) {
            $dat = sqlDate($di->time - $di->config->get('clear_debug_log_days') * 3600 * 24);
            $di->errorLogTable->clearOldDebug($dat);
        }

        if ($di->config->get('clear_inc_payments') && $di->config->get('clear_inc_payments_days') > 0) {
            $dat = sqlDate($di->time - $di->config->get('clear_inc_payments_days') * 3600 * 24);
            $di->invoiceTable->clearPending($dat);
        }

        if ($di->config->get('clear_inc_users') && $di->config->get('clear_inc_users_days') > 0) {
            $dat = sqlDate($di->time - $di->config->get('clear_inc_users_days') * 3600 * 24);
            $di->userTable->clearPending($dat);
        }

        if ($di->config->get('clear_invoice_log') && $di->config->get('clear_invoice_log_days') > 0) {
            $dat = sqlDate($di->time - $di->config->get('clear_invoice_log_days') * 3600 * 24);
            $di->invoiceLogTable->clearOld($dat);
        }

        $di->uploadTable->cleanUp();
        $di->mailQueue->cleanUp();
        $di->cron->cleanUp();
        $di->tasksStorage->cleanUp();
    }
    ...
    function initConstants()
    {
        ...
        if (!defined('ROOT_DIR'))
        {
            define('ROOT_DIR', dirname(realpath(__FILE__), 3));
        }
