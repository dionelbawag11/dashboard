...
class SendpassController extends Am_Mvc_Controller
{
    ...
    function loadUser(string $login) : ?User
    {
        ...
        if (!$user) {
            $event = new Am_Event(Am_Event::AUTH_LOST_PASS_USER_EMPTY, ['login' => $login]);
    ...
    public function doSend()
    {
        ...
        do {
            ...
            if($this->getDi()->config->get('enable-otp') && ($adapter = $user->getOtpAdapter()))
            {
                $session = Am_Otp_Session::start($user, $this->getDi()->surl('login'));
                $url = $session->getSendCodeUrl();
                if ($this->_request->isXmlHttpRequest())
                {
                    ...
                    return Am_Mvc_Response::redirectLocation($session->getSendCodeUrl());
                }
...
CUT;
    ...
    protected function checkCode($code, &$user)
    {
        ...
        [$user_id, $pass, $email, $last_login] = explode('|', $data, 4);
        $user = $this->getDi()->userTable->load($user_id);

        if ($user->pass != $pass || $user->email != $email || $user->last_login != $last_login)
            return false;
    ...
    public function sendSecurityCode(User $user)
    {
        ...
        $data = [$user->pk(), $user->pass, $user->email, $user->last_login];
