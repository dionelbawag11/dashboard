...
class SignupController extends Am_Mvc_Controller
{
    ...
    function loadForm()
    {
        if ($c = $this->getFiltered('c')) {
            if ($c == 'cart') {
                if ($this->_request->getParam('amember_redirect_url'))
                    $this->getSession()->redirectUrl = amFilterAmemberRedirectUrl($this->_request->getParam('amember_redirect_url'));
        ...
        if ($this->record->is_disabled) {
            throw new Am_Exception_InputError('Signup form is Disabled');
        }
    ...
    function process(array $vars, $name, HTML_QuickForm2_Controller_Page $page)
    {
        ...
        // keep reference to e-mail confirmation link so it is still working after signup
        ...
        if (!$invoice->getItems()) {
            ...
            Am_Mvc_Response::redirectLocation($this->user->is_approved ?
                $this->getDi()->url('login?signup', false) :
