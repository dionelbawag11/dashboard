...
function ___($msg)
{
    ...
    $msg = $args ? preg_replace('/%(\s|$)/', '%%\1', $msg) : $msg;
...
/**
 * Checks if redirect for amember_redirect_url is possible
 * @param string $url
 * @return string|void
 */
function amFilterAmemberRedirectUrl($url) {
    $url = str_replace('\\', '/', $url);
    ...
    if (array_key_exists('host', $redirect_url) && !Am_Di::getInstance()->config->get('other_domains_redirect')) {
        $match = false;
