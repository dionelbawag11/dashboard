...
abstract class Am_Pdf_Invoice_Abstract
{
    ...
    const LOGO_DEFAULT_HEIGHT = 100;

    const NEW_PAGE_THRESHOLD = 80;
    ...
    public function drawDefaultTemplate(Zend_Pdf_Page $p)
    {
        $padd = 40;

        $pointer = $this->getPaperHeight() - $padd;

        $page = new Am_Pdf_Page_Decorator($p);
        ...
        if ($invoice_logo_id && ($upload = $this->getDi()->uploadTable->load($invoice_logo_id, false))) {
            if (file_exists($upload->getFullPath())) {
                ...
                if ($image) {
                    $gh = $this->getDi()->config->get('invoice_logo_height', self::LOGO_DEFAULT_HEIGHT);
                    $gw = $this->getDi()->config->get('invoice_logo_width', self::LOGO_DEFAULT_HEIGHT);
    ...
    function newPage($pdf)
    {
        if ($this->getDi()->config->get('invoice_custom_template') &&
            ...
            $templ = Zend_Pdf::load($upload->getFullPath());

            $pdf->pages[] = $_ = clone $templ->pages[0];

            $this->pointer = $this->getPaperHeight() - $this->getDi()->config->get('invoice_skip', 150);
        } else {
            $pdf->pages[] = $_ = $pdf->newPage($this->getDi()->config->get('invoice_format', Zend_Pdf_Page::SIZE_LETTER));

            $this->pointer = $this->drawDefaultTemplate($_);
        }
    ...
    public function getFileName()
    {
        ...
        return str_replace('/', '_', $tmp->render($filename));
    }
    ...
    public function getCountry($invoice)
    {
