...
class Am_Form_Element_CreditCardExpire extends HTML_QuickForm2_Container_Group
{
    public function __construct($name = null, $attributes = null, $data = null)
    {
        ...
        $this->setSeparator(' ');
        $require = !isset($data['dont_require']) || !$data['dont_require'];
        $years = @$data['years'];
        if (!$years) $years = 15;
    ...
    public function getYearOptions($add){
        ...
        return ['' => ___('Year')] + array_combine($years, $years);
    }
...
class Am_Form_CreditCard extends Am_Form
{
    ...
    public function init() {
        ...
        $name_f = $name->addText('cc_name_f', ['size'=>15]);
        ...
        $name_l = $name->addText('cc_name_l', ['size'=>15]);
        ...
        if (in_array(Am_Paysystem_CreditCard::CC_TYPE_OPTIONS, $options)) {
            $type = $this->addSelect('cc_type')
                ->setLabel(___("Credit Card Type\n" .
                    'please select one'))
                ->loadOptions(array_merge([''=>'-- ' . ___('Please choose') . ' --'],
        ...
        if ($this->formType == self::ADMIN_UPDATE) {
            ...
            $group->addStatic('cc');
            $cc = $group->addText('cc_number', ['autocomplete'=>'off', 'size'=>22, 'maxlength'=>22, 'style'=>'display:none']);
            $cc->addRule('regex', ___('Invalid Credit Card Number'), '/^[0-9 -]+$/')
                ->addRule('callback2', 'Invalid CC#', [$this, 'validateCreditCardNumber']);
...
CUT
            ...
            $cc = $this->addText('cc_number', ['autocomplete'=>'off', 'size'=>22, 'maxlength'=>22])
                    ->setLabel(___("Credit Card Number\n" .
                        'for example: 1111-2222-3333-4444'));
            $cc->addRule('required', ___('Please enter Credit Card Number'))
                ->addRule('regex', ___('Invalid Credit Card Number'), '/^[0-9 -]+$/')
                ->addRule('callback2', 'Invalid CC#', [$this->plugin, 'validateCreditCardNumber']);
        }
        ...
        if (!in_array($this->formType, [self::ADMIN_INSERT, self::ADMIN_UPDATE]) &&
            in_array(Am_Paysystem_CreditCard::CC_CODE, $options)) {
        ...
        if (in_array(Am_Paysystem_CreditCard::CC_MAESTRO_SOLO_SWITCH, $options)) {
            $issue = $this->addText('cc_issuenum', ['autocomplete'=>'off', 'size'=>20, 'maxlength'=>22])
                    ->setLabel(___("Card Issue #\n" .
                        'is required for Maestro/Solo/Switch credit cards only'))
                    ->addRule('regex', ___('Invalid Issue Number'), '/^\d+$/');
            $this->addElement(new Am_Form_Element_CreditCardExpire('cc_startdate', null, ['dont_require'=>true, 'years'=>-10]))
        ...
        if (in_array(Am_Paysystem_CreditCard::CC_ADDRESS, $options)) {
            ...
            if (in_array(Am_Paysystem_CreditCard::CC_STREET, $options)) {
                $street = $fieldSet->addText('cc_street', ['class'=>'am-el-wide'])->setLabel(___('Street Address'))
                                   ->addRule('required', ___('Please enter Street Address'));
            }
            if (in_array(Am_Paysystem_CreditCard::CC_STREET2, $options)) {
                $street2 = $fieldSet->addText('cc_street2', ['class'=>'am-el-wide'])->setLabel(___('Street Address (Second Line)'))
                               ->addRule('required', ___('Please enter Street Address'));
            }
            if (in_array(Am_Paysystem_CreditCard::CC_HOUSENUMBER, $options)) {
                $house = $fieldSet->addText('cc_housenumber', ['size'=>15])->setLabel(___('Housenumber'))
            ...
            if (in_array(Am_Paysystem_CreditCard::CC_PROVINCE_OUTSIDE_OF_US, $options)) {
                $province = $fieldSet->addText('cc_province', ['size'=>15])
            ...
            if (in_array(Am_Paysystem_CreditCard::CC_PHONE, $options)) {
                $phone = $fieldSet->addText('cc_phone')->setLabel(___('Phone'))
        ...
        elseif ($this->formType == self::PAYFORM) {
            $buttons->addStatic()
                ->setContent(sprintf('&nbsp;&nbsp;<a href="'.$this->plugin->getCancelUrl().'">%s</a>', ___("Cancel")));
        }
        $this->plugin->onFormInit($this);
    }

    function validateCreditCardNumber($cc_number)
    {
        if(!empty($cc_number))
            $this->plugin->validateCreditCardNumber($cc_number);
    }
    /**
