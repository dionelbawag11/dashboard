...
class Invoice extends Am_Record_WithData
{
    ...
    const UPGRADE_TYPE = 'upgrade-type';
...
//                    return $this->_getItemCompatibleError('SECONDPERIOD', $item, $existingItem);
    ...
    function render($indent = "", InvoicePayment $payment = null)
    {
        ...
        if ($invoice->{$prefix . '_tax'} > 0) {
            foreach ($taxes as $rate => $tax) {
                $out .= $indent . sprintf("{$total_space}%-{$column_total}s{$space}%{$price_width}s$newline", ___('Tax') . sprintf(' (%s%%)', $rate), Am_Currency::render($tax, $invoice->currency));
            }
        }
    ...
    public function cancelUpgradedInvoice($invoice_id)
    {
        ...
        if ($this->data()->get(self::UPGRADE_TYPE) != ProductUpgrade::TYPE_NEXT_CYCLE) {
            // stop access records for invoice_item_id
            if ($item) {
                $activeAccess = $this->getDi()->accessTable->selectObjects("SELECT *
                FROM ?_access
                WHERE invoice_id=?d AND product_id=?d AND expire_date > ?", $invoice_id, $item->item_id,
                    $this->getDi()->sqlDate);
        ...
        elseif($this->first_total <= 0)
        {
            //add access to new invoice for product from old invoice
            //single access is enough to properly calculate rebill date using addToRebillDate
            if ($item) {
                if($activeAccess = $this->getDi()->accessTable->selectObjects("SELECT *
                FROM ?_access
                WHERE invoice_id=?d AND product_id=?d AND expire_date > ? LIMIT 1", $invoice_id, $item->item_id,
                    $this->getDi()->sqlDate))
                {
                    $access = $activeAccess[0];
                    unset($access->access_id);
                    $access->invoice_id = $this->pk();
                    $access->insert();
                }
            }
        }
        ...
        if ($this->data()->get(self::UPGRADE_TYPE) == ProductUpgrade::TYPE_NEXT_CYCLE) {
            foreach ($this->getItems() as $item) {
                $item->data()->set('no-access', null)->update();
            }
        }
    }
        ...
        foreach ($this->getItems() as $item) {
            if ($item->rebill_times) {
                if ($product = $item->tryLoadProduct()) {
                    $expires = $this->getDi()->db->selectCell(
                        "SELECT MAX(expire_date) FROM ?_access WHERE user_id=? AND product_id=?",
                        $this->user_id,
                        $product->pk()
                    );
                    break;
 ...
 * @method Invoice[] selectObjects($sql, $param1 = null)
 * @method Invoice load($id, $throwException=true)
 */
