...
abstract class Am_Paysystem_Nmi extends Am_Paysystem_CreditCard
{
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        if($this->useKey)
        {
            $form->addSecretText('security_key')
                ->setLabel("API Security Key\n" .
    ...
    {
        if($this->useKey)
            return $this->getConfig('security_key');
        else
    ...
    public function _doBill(Invoice $invoice, $doFirst, CcRecord $cc, Am_Paysystem_Result $result)
    {
        ...
        if ($doFirst) // not recurring sale
        {
            if (!(float)$invoice->first_total) // first - free
            {
                ...
                {
                    return $result->setFailed(["NMI Plugin: Bad auth response."]);
                }
        ...
        } else
        {
            ...
            if (!$customerVaultId)
            {
                return $result->setFailed(["No saved reference transaction for customer"]);
            }
    ...
    public function storeCreditCard(CcRecord $cc, Am_Paysystem_Result $result)
    {
        ...
        if (!$customerVaultId)
        {
            ...
            if (!$customerVaultId)
            {
                return $result->setFailed(["PSWW Plugin: Bad add response."]);
            }
            $user->data()->set($this->getCustomerVaultVariable(), $customerVaultId)->update();
        }
