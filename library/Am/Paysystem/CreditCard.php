<?php
use Freelancehunt\Validators\CreditCard as CreditCardValidator;
...
abstract class Am_Paysystem_CreditCard extends Am_Paysystem_Abstract
{
    ...
    const LAST_REBILL_FAILURE = 'last_rebill_failure';
    ...
    {
        return new Am_Form_CreditCard($this, $actionName);
    }
    ...
    {
        return ($this->getRecurringType() == self::REPORTS_CRONREBILL);
    }

    function isManualRebillPossible(Invoice $invoice)
    {
        $d = intval($this->getConfig('manual_rebill_grace_period', 30));
        $manualRebillGracePeriod = $this->getDi()->dateTime->modify("-{$d} days")->format('Y-m-d');

        return $this->supportsManualRebill()
            && $this->getConfig('manual_rebill_enabled')
            && in_array($invoice->status, [Invoice::RECURRING_ACTIVE, Invoice::RECURRING_FAILED])
            && $invoice->data()->get(self::FIRST_REBILL_FAILURE)
            && $invoice->data()->get(self::FIRST_REBILL_FAILURE) <= $this->getDi()->sqlDate
            && $invoice->data()->get(self::LAST_REBILL_FAILURE) >= $manualRebillGracePeriod
            && $invoice->getPaymentsCount() < $invoice->getExpectedPaymentsCount();
    }
    ...
    protected function ccActionValidateSetInvoice(Am_Mvc_Request $request, array $invokeArgs)
    {
        ...
        if ($invoice->isCompleted() && !$this->isManualRebillPossible($invoice))
            Am_Mvc_Response::redirectLocation($this->getDi()->surl('member'));

        if ($invoice->paysys_id != $this->getId())
            throw new Am_Exception_InputError("You have used wrong link for payment page, please return back and try again");

        $due_date = $invoice->due_date ?: sqlDate("{$invoice->tm_added} +" . Invoice::DEFAULT_DUE_PERIOD . " days");

        if ($due_date < sqlDate('now') && !$this->isManualRebillPossible($invoice))
            throw new Am_Exception_InputError("Invoice expired");
    ...
    {
        $ret = [self::CC_CODE, self::CC_ADDRESS, self::CC_COUNTRY, self::CC_STATE, self::CC_CITY, self::CC_STREET, self::CC_ZIP];
    ...
    {
        $ret = CreditCardValidator::validCreditCard($cc);
        if (!$ret['valid'])
            return ___('Invalid Credit Card Number');
        return null;
    }
    ...
    public function doMaxmindCheck(Invoice $invoice, CcRecord $cc)
    {
        ...
        $ps->postal = $cc->cc_zip;
        [$acc, $domain] = @explode('@', $user->email, 2);
        $ps->domain = $domain;
    ...
    {
        if ($this->storesCcInfo())
            return $this->getDi()->ccRecordTable->findFirstByUserIdPaysysId($invoice->user_id, $invoice->paysys_id);
    }
    ...
    public function prorateInvoice(Invoice $invoice, CcRecord $cc, Am_Paysystem_Result $result, $date)
    {
        ...
        $days_diff = round((strtotime($date) - strtotime($first_failure)) / (24*3600));
    ...
    public function onRebillFailure(Invoice $invoice, CcRecord $cc, Am_Paysystem_Result $result, $date)
    {
        ...
        $invoice->data()->set(self::LAST_REBILL_FAILURE, $date)->update();
        ...
        try
        {
            if($et = Am_Mail_Template::load('cc.rebill_failed'))
            {
                ...
                $et->setManual_rebill_link(
                    $this->isManualRebillPossible($invoice) ? sprintf(___("Follow this link to complete subscription payment manually: %s"), $this->getManualRebillUrl($invoice)) : ""
                );
            ...
            $this->getDi()->logger->error("Could not sendRebillFailedToUser invoice#{$invoice->public_id}", ["exception" => $e]);
        }
    }
    ...
    function sendRebillSuccessToUser(Invoice $invoice)
    {
        ...
        {
            // No mail exceptions when  rebilling;
            $this->getDi()->logger->error("Could not sendRebillSuccessToUser invoice#{$invoice->public_id}", ["exception" => $e]);
        }
    }
    ...
    public function onRebillSuccess(Invoice $invoice, CcRecord $cc, Am_Paysystem_Result $result, $date)
    {
        ...
        $invoice->data()->set(self::LAST_REBILL_FAILURE, null)->update();
    ...
    {
        if (defined('AM_DISABLE_CC_REBILL') && AM_DISABLE_CC_REBILL)
        {
            Am_Di::getInstance()->logger->notice(__METHOD__ . " disabled because of AM_DISABLE_CC_REBILL const in config");
            return;
        }
        /**
        ...
        foreach ($this->getDi()->invoiceTable->findForRebill($date, $this->getId()) as $invoice)
        {
            ...
            /* @var $invoice Invoice */
            try {
                $rebill = $rebillTable->createRecord([
                ...
                $this->getDi()->logger->error("Error during cc-rebill", ["exception" => $e]);
        ...
        $this->getDi()->hook->call(Bootstrap_Cc::EVENT_CC_AFTER_REBILL, [
           'paysys_id' => $this->getId(),
           'date' => $date,
        ]);
    }
    ...
    protected function getStatisticsRow(Array $r)
    {
        ...
        return sprintf("%s, %s, %s %s, \nInvoice: %s\tAmount: %s\t%s\n%s\n\n",
            ...
            $r['second_total'],
            $failed,
            $this->getDi()->surl(['admin-user-payments/index/user_id/%s#invoice-%s', $r['user_id'], $r['invoice_id']])
        );
    }
    ...
    protected function sendStatisticsEmail()
    {
        ...
        if ($et = Am_Mail_Template::load('cc.admin_rebill_stats'))
        {
            foreach($this->getDi()->db->selectPage($total, "
                SELECT r.*, IF(r.status = ?, ip.amount, i.second_total) AS second_total, i.user_id, i.invoice_id, i.public_id, i.rebill_date, u.name_f, u.name_l, u.email, u.login
                    FROM ?_cc_rebill r
                    LEFT JOIN ?_invoice i USING (invoice_id)
                    LEFT JOIN ?_user u ON (i.user_id = u.user_id)
                    LEFT JOIN ?_invoice_payment ip ON ip.invoice_id=i.invoice_id AND DATE(ip.dattm) = DATE(r.status_tm)
                WHERE status_tm>? AND status_tm<=? AND r.paysys_id=?
                ", CcRebill::SUCCESS, $date, $this->getDi()->sqlDateTime, $this->getId()) as $r)
            {
                if ($r['status'] == CcRebill::SUCCESS) {
    ...
    protected function _afterInitSetupForm(Am_Form_Setup $form)
    {
        ...
        if($this->supportsManualRebill())
        {
            $fs->addAdvCheckbox("manual_rebill_enabled")->setLabel(___("Enable Manual Rebill Support
            Allow for user to complete rebill manually if automatic rebill failed for some reason"));
            $fs->addText('manual_rebill_grace_period', ['size'=>3])->setLabel("Manual Rebill Grace Period ..., days
            Period when user will be able to complete manual rebill after automatic rebill fails");
            $form->setDefault('manual_rebill_enabled', 1);
            $form->setDefault('manual_rebill_grace_period', 30);
            $fs->addScript()->setScript(<<<EOT
jQuery(function(){
    jQuery("[id^=manual_rebill_enabled-]").on("change", function(e){
        jQuery("[id^=manual_rebill_grace_period-]").closest('.am-row').toggle(jQuery(this).prop("checked"));
    }).change();
});
...
EOT
        ...
        if($this->getRecurringType() == self::REPORTS_CRONREBILL)
        {
            $el = $fs->addMagicSelect('reattempt', ['multiple'=>'multiple']);
            $options = [];
        ...
        {
            $fs->addFieldset()->setLabel(___('MaxMind Credit Card Fraud Detection'));
            $fs->addAdvCheckbox('use_maxmind')->setLabel(___('Use MaxMind Credit Card Fraud Detection'));
            $fs->addText('maxmind_license_key')->setLabel(
            ...
            $fs->addSelect('maxmind_requested_type')->setLabel(
                            ___("Requested Type\n" .
            ...
            $fs->addText('maxmind_risk_score')->setLabel(
                    ___("Risk Score\n" .
            ...
            $fs->addAdvCheckbox('maxmind_use_number_identification')->setLabel(
            ...
            $fs->addMagicSelect('maxmind_tni_phone_types')->setLabel(
            ...
            $fs->addAdvCheckbox('maxmind_allow_country_not_matched')->setLabel(
            ...
            $fs->addAdvCheckbox('maxmind_allow_high_risk_country')->setLabel(
            ...
            $fs->addAdvCheckbox('maxmind_allow_anonymous_proxy')->setLabel(
...
CUT
        ...
        if (!preg_match('#^const:#', AM_KEYFILE) && !is_readable(AM_KEYFILE))
        {
...
CUT
    ...
    {
        if ($this->storesCcInfo() &&
            $this->getDi()->ccRecordTable->findFirstByUserIdPaysysId($user->user_id, $this->getId()))
        {
            return $this->getPluginUrl('update');
        }
    }

    public function getCardElementId()
    {
        return 'cc_number-0';
    }
    ...
    /**
     * @param Invoice $invoice
     * @return CcRecord[] stored cards array, keys do not matter
     */
