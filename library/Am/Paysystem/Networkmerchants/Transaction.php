...
class Am_Paysystem_Networkmerchants_Transaction extends Am_Paysystem_Transaction_CreditCard
{
    ...
    {
        return (!$this->plugin->getConfig('testMode')) ? $this->plugin->getConfig('security_key') : '6457Thfj624V5r7WUwc5v6a68Zsd6YEm';
    }

    protected function addRequestParams()
    {
        if($this->plugin->isUseKey())
            $this->request->addPostParameter('security_key', $this->getSecurityKey());
        else {
    ...
    public function validate()
    {
        ...
        if (!empty($err))
        {
            return $this->result->setFailed([$err, $this->parsedResponse->responsetext]);
        }
