...
CUT
    ...
    public function supportsChangeRebillDate()
    {
     ...
     * @param Invoice $invoice
     * @param $rebill_date
     * @throws Exception $ex  exception in case of rebill date wasn't changed
     */
    public function changeRebillDate(Invoice $invoice, DateTime $rebill_date)
    {
        if(!$this->supportsChangeRebillDate())
            throw new Am_Exception_InputError(___("Plugin doesn't allow to change rebill date"));

        $invoice->updateQuick('rebill_date',  $rebill_date->format('Y-m-d'));

        // Extend an access accordingly
