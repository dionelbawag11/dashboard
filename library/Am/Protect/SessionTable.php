...
class Am_Protect_SessionTable extends Am_Table implements Am_Protect_SingleLogin
{
    ...
    protected $_defaultCookieParams = [
        'expires' => 0,
        'path' => '/',
        'domain' => null,
        'secure' => false,
    ...
    {
        $r = $this->findFirstBy([$this->_sid => $sid]);
        if (!$r)
            return null;
    ...
    function setCookie($name, $value)
    {
        ...
        if (!is_null($this->_cookie_params))
            foreach (['expires', 'path', 'domain', 'secure', 'exact_domain'] as $k)
        ...
        call_user_func_array(["Am_Cookie", "set"], $params);
    }
    ...
    {
        $params = [$name, '', time() - 24 * 3600];
        if (!is_null($this->_cookie_params))
            foreach (['path', 'domain', 'secure', 'exact_domain'] as $k)
                $params[] = array_key_exists($k, $this->_cookie_params) && isset($this->_cookie_params[$k]) ? $this->_cookie_params[$k] : $this->_defaultCookieParams[$k];
        call_user_func_array(["Am_Mvc_Controller", "setCookie"], $params);
    }
    ...
    function getLoggedInRecord()
    {
        ...
        return $this->_plugin->getTable()->load($uid, false);
    }
