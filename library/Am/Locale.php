 ...
 *
 * @package Am_Utils
 * @method formatTermsText(Am_TermsText $terms)
 * @method formatOptionTermsText(Am_TermsText $terms)
 * @method formatPeriod(Am_Period $period, $format="%s", $skip_one_c = false)
 */
class Am_Locale
{
    protected $locale = '';
    ...
    static function _readData($locale)
    {
        ...
        {
            if(!isset($header[$locale])) continue;
...
abstract class Am_Locale_Formatter_Base implements Am_Locale_Formatter
{
    ...
    public function formatTermsText(Am_TermsText $terms)
    {
        ...
        if (!$p1->isLifetime()) {
            if ($terms->rebill_times) {
                $ret .= $p1->getText($this->f('bp_for_first', $p1->getUnit()), true);
            } else {
                $ret .= $p1->getText($this->f('bp_for', $p1->getUnit()));
            }
        }
        if ($terms->rebill_times)
        {
            ...
            if($terms->rebill_times == 1 && !$equal) {
                $ret .= (string)$c2 . $p2->getText($this->f('bp_for', $p2->getUnit()));
            } else {
                $ret .= (string)$c2 . $p2->getText($this->f('bp_for_each', $p2->getUnit()), true);
            }
    ...
    public function formatPeriod(Am_Period $period, $format="%s", $skip_one_c = false)
    {
        ...
        $cc = $period->getCount();
        if ($period->getCount() == 1) $cc = $skip_one_c ? '' : $this->f('period_one', $period->getUnit());
    ...
    protected function f($token, $unit)
    {
        return $this->f["{$token}_{$unit}"] ?? $this->f[$token];
    }
...
class Am_Locale_Formatter_de extends Am_Locale_Formatter_Base
{
    protected $f = [
        ...
        'bp_for_first_d' => [' für den ersten %s', ' für die ersten %s'],
        'bp_for_first_m' => [' für den ersten %s', ' für die ersten %s'],
        'bp_for_first_y' => [' für das erste %s', ' für die ersten %s'],

        'bp_for_each' => [' für jedes %s', ' für alle %s'],
        'bp_for_each_d' => [' jeden %s', ' alle %s'],
        'bp_for_each_m' => [' jeden %s', ' alle %s'],
        'bp_for_each_y' => [' jedes %s', ' alle %s'],

        'bp_then' => ', danach ',
        'bp_installments' => [', für %d Rate', ', für %d Raten'],
    ];
...
class Am_Locale_Formatter_nb extends Am_Locale_Formatter_Base
{
    protected $f = [
        'period_day' => ['dag', 'dager'],
        'period_month' => ['måned', 'måneder'],
        'period_year' => ['år', 'år'],
        'period_lifetime' => ' for livsvarig',
        'period_up_to' => ' opp til %s',
        'period_one' => 'en',
        ...
        'bp_for_each' => ' for hver %s',
        'bp_then' => ', deretter ',
        'bp_installments' => [', for %d delbetaling', ', for %d delbetalinger'],
    ];
