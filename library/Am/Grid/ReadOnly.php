...
class Am_Grid_ReadOnly
{
    ...
    function renderTotal($fromCache=true)
    {
        $msgs = [];
        $total = $this->getFoundRows($fromCache);
        ...
        if ($total == 1) {
            $msgs[] = ___("showing %s record", number_format($total));
        ...
        return sprintf("<span class='am-grid-title-desc__total'>%s</span>", implode("", $msgs));
    }
    ...
    function renderTitle($noTags = false)
    {
        ...
        $msgs = [];
        $msgs[] = $this->canCacheTotals() ? sprintf(
            "<span class='need-reload' data-url='%s' data-key='total'>%s</span>",
            $this->makeUrl([
                self::ACTION_KEY => 'reload'
            ]),
    ...
    {
        return [
            'totalRecords' => $this->getFoundRows(),
    ...
    function renderPaginator($fromCache=true, $urlTemplate=null)
    {
        $total = $this->getFoundRows($fromCache);
        ...
        return $fromCache ? sprintf("<span class='need-reload' data-url='%s' data-key='paginator'>%s</span>", $this->makeUrl([
            self::ACTION_KEY => 'reload',
    ...
    function renderContent()
    {
        ...
        $out =
            $this->renderFilter() .
            $table .
            $this->renderPaginator($this->canCacheTotals());
    ...
    {
        $url = $this->makeUrl([self::ACTION_KEY => 'index', 'p'=>null]);
        $insert = urlencode($this->id.'_p').'=###PAGE###' ;
        $url .= (strpos($url, '?')!==false) ? ('&'. $insert) : ('?'.$insert);
        $this->getDi()->session->writeClose();
        $this->getDi()->response->ajaxResponse([
            'total' => $this->renderTotal(false),
            'paginator' => $this->renderPaginator(false, $url)
        ]);
    }
    ...
    function canCacheTotals()
    {
        return AM_HUGE_DB && method_exists($this->getDataSource(), 'getCacheId');
    }

    /**
     * Get found rows from cache, if not exists get it from datasource and cache it;
     */
    function getFoundRows($fromCache = true)
    {
        if($this->canCacheTotals() && $fromCache)
        {
            return $this->getDi()->cache->load($this->getDataSource()->getCacheId('long-time'));
        }

        $foundRows = $this->dataSource->getFoundRows();

        if($this->canCacheTotals())
        {
            $this->getDi()->cache->save($foundRows, $this->getDataSource()->getCacheId('long-time'), [AM_HUGE_DB_CACHE_TAG], AM_HUGE_DB_CACHE_TIMEOUT * 24 * 10);
        }
