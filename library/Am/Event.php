...
class Am_Event
{
    ...
    function __construct($id = null, array $vars = [])
    {
        $this->id = $id;
        $this->vars = $vars;
    }
...
abstract class Am_Event_User extends Am_Event
{
    ...
    {
        parent::__construct(null, ['user' => $user]);
    }
}
...
class Am_Event_InvoiceGetCalculators extends Am_Event
{
    ...
    {
        parent::__construct(null, ['invoice' => $invoice]);
    }
    ...
    function insertBefore(Am_Invoice_Calc $calc, Am_Invoice_Calc $before)
    {
        ...
        foreach ($this->return as $k => $v)
            if ($v === $before)
            {
                array_splice($this->return, $k, 0, [$calc]);
                return true;
            }
    }
    ...
    function insertAfter(Am_Invoice_Calc $calc, Am_Invoice_Calc $after)
    {
        ...
        foreach ($this->return as $k => $v)
            if ($v === $after)
            {
                array_splice($this->return, $k+1, 0, [$calc]);
                return true;
            }
    }
...
class Am_Event_SubscriptionChanged extends Am_Event
{
    ...
    {
        parent::__construct(null, ['user' => $user, 'added' => $added, 'deleted' => $deleted]);
    }
}
...
class Am_Event_Grid extends Am_Event
{
    ...
    protected $grid;
    protected $args = [];
    public function __construct($action, array & $args, Am_Grid_ReadOnly $grid)
    {
        parent::__construct();
        $this->action = $action;
        $this->grid = $grid;
...
class Am_Event_UserNoteAfterInsert extends Am_Event
{
    protected $user, $note;

    function __construct(UserNote $note)
    {
        $this->note = $note;
    }
    ...
    {
        if (empty($this->user)) {
            $this->user = $this->getDi()->userTable->load($this->getNote()->user_id);
        }
...
class Am_Event_UserNoteAfterUpdate extends Am_Event
{
    protected $user, $note;
