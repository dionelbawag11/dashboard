...
{
    static private $availableReports = [];
    /** @var Am_Form_Admin */
    protected $form;
    ...
    protected function createForm()
    {
        ...
        $form->addSubmit('save', ['value'=>___('Run Report')]);
        return $form;
    }
...
abstract class Am_Report_Date extends Am_Report_Abstract
{
    ...
    protected function _initConfigForm(Am_Form $form)
    {
        $period = $form->addSelect('period')
            ->setLabel(___('Period'))
            ->loadOptions(
                    array_merge($this->getDi()->interval->getOptions(), [self::PERIOD_EXACT => ___('Exact')]));
...
CUT;
        ...
        $g = $form->addGroup(null, ['class' => 'am-row-required'])
            ->setSeparator(' ')
            ->setLabel(___('Start and End Dates'));

        $g->addDate('start')
            ->addRule('required');
        $g->addHtml()->setHtml('&mdash;');
        $g->addDate('stop')
            ->addRule('required');
        $form->addRule('callback', 'Start Date cannot be later than the End Date', [$this, 'checkStopDate']);
        $form->addSelect('quant')
            ->setLabel(___('Grouping'))
            ->loadOptions($this->getQuantityOptions())
            ->addRule('required');
    }
    ...
    protected function _afterInitConfigForm(Am_Form $form)
    {
        foreach (Am_Report_Shift::getOptions() as $k => $v) {
            ...
            $form->addAdvCheckbox('shift_' . $k, [
                'rel' =>
                    implode(' ', $s->getCompatiblePeriods())
            ], ['content' => $v]);
        }
        $form->addScript()
            ->setScript(<<<CUT
...
CUT
    ...
    {
        if ($val['period'] !== self::PERIOD_EXACT) return true;
    ...
    public function getReport(Am_Report_Result $result = null, Am_Report_Shift $shift = null)
    {
        ...
        //set start and end points (it can not exists in query result)
        $r = [];
        foreach ($result->getLines() as $line) {
            $r[$line->getKey()] = 0;
        }

        if ($this->start != date('Y-m-d 00:00:00', 0)) {
            $q = $this->getQuantity()->getSqlExpr("'{$this->start}'");
            $first_point = $this->getDi()->db->selectCell("SELECT {$q};");
            $result->addValues($first_point, $this->getQuantity()->getLabel($first_point), $r);
        }

        if ($this->stop != '2037-12-31 23:59:59') {
            $q = $this->getQuantity()->getSqlExpr("'{$this->stop}'");
            $last_point = $this->getDi()->db->selectCell("SELECT {$q};");
            $result->addValues($last_point, $this->getQuantity()->getLabel($last_point), $r);
        }
        ...
        foreach ($this->shift as $id) {
            ...
            if (!in_array($this->period, $shift->getCompatiblePeriods())) continue;
            $diff = '-' .  $shift->getDiff();
...
abstract class Am_Report_Shift
{
    ...
    abstract function getCompatiblePeriods();
}
...
abstract class Am_Report_Quant
{
    ...
    public function buildQuery(Am_Query $q, $pointField, Am_Report_Abstract $report)
    {
...
abstract class Am_Report_Quant_Date extends Am_Report_Quant
{
    ...
    /**
     * retrieve start and stop datetime for given key in SQL DATE Format
     *
     * @return array
     * @param string $key SQL point value
     */
...
class Am_Report_Quant_Day extends Am_Report_Quant_Date
{
    ...
    {
        $date = sqlDate($key);
        return [sprintf('%s', $date),sprintf('%s', $date)];
    }
...
class Am_Report_Quant_Week extends Am_Report_Quant_Date
{
    ...
    public function getStartStop($key)
    {
        ...
        return [sqlDate($start),sqlDate($start + 7*24*3600 - 1)];
    }
...
class Am_Report_Quant_Month extends Am_Report_Quant_Date
{
    ...
    public function getLineAxisParams()
    {
        return ['xLabels' => 'month'];
    }

    public function getStartStop($key)
    {
        $start = $this->getStart($key);
        return [sqlDate($start),sqlDate(strtotime(sprintf('%s +1 month', date('Y-m', $start)))-1)];
    }
...
class Am_Report_Quant_Quarter extends Am_Report_Quant_Date
{
    ...
    {
        $start = $this->getStart($key);
        return [sqlDate($start),sqlDate(strtotime('+3 month', $start)-1)];
    }
...
class Am_Report_Quant_Year extends Am_Report_Quant_Date
{
    ...
    public function getStartStop($key)
    {
        return [sprintf('%s-01-01', $key),sprintf('%s-12-31', $key)];
    }
...
class Am_Report_Line
{
    ...
    public function setShift($shift) { $this->shift = $shift; }
...
class Am_Report_Table extends Am_Report_Output
{
    ...
    public function render()
    {
        ...
        foreach ($this->result->getLines() as $line)
            $out .= "<th class='align-right'>" . Am_Html::escape($line->getLabel()) . "</th>\n";
        $out .= "</tr>\n";
        $lines = $totals = $vals = [];
        foreach ($this->result->getPoints() as $point)
        {
            ...
            foreach ($this->result->getLines() as $line)
            {
                ...
                $lines[$i] = $line;
                $totals[$i] = ($totals[$i] ?? 0) + (float) $point->getValue($line->getKey());
                $i++;
            }
        }
        $rn = 0;
        foreach ($vals as $pointLabel => $v) {
            ...
            foreach ($v as $i => $lineV)
            {
                $out .= sprintf('<td style="text-align: right"><span title="%s">%s</span></td>',
                    ($totals[$i] > 0) ?
                        Am_Html::escape(round(floatval($lineV)*100/$totals[$i], 2) . '%') :
                        '',
                    Am_Html::escape($lines[$i]->formatValue(floatval($lineV))));
            }
            $out .= "</tr>\n";
        }
        if ($this->addTotal) {
    ...
    public function render()
    {
        ...
        foreach ($this->result->getPoints() as $point)
        {
            ...
            foreach ($this->result->getLines() as $line)
            {
                ...
                @$lines[$i] = $line;
                @$totals[ $i++ ] += (float)$point->getValue($line->getKey());
            }
            $out .= "\r\n";
        }
