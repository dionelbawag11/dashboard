...
class Am_Paysystem_Stripe extends Am_Paysystem_CreditCard_Token implements Am_Paysystem_TokenPayment
{
    ...
    protected function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addAdvCheckbox('enable_paymethod_select')->setLabel(
            ___("Show existing payment methods select\n" .
                "if existing Stripe customer will try to signup " .
                "plugin will offer to pay using payment method already registered"));
...
CUT;
    ...
    {
        if($this->findInvoiceForTokenUpdate($user->pk()) && $this->getDi()->invoiceTable->findFirstBy([
                'user_id' => $user->pk(),
...
CUT
    ...
    function insertPaymentFormBrick(HTML_QuickForm2_Container $form)
    {
        ...
        if(!empty($token[self::CUSTOMER_ID]) && $this->getConfig('enable_paymethod_select'))
        {
            // Existing user, load payment methods to display them
            $tr = new Am_Paysystem_Transaction_Stripe_ListPaymentMethods($this, $this->invoice);
            ...
            if(!empty($methods))
            {
                $form->addAdvRadio('payment_method', ['id' => 'paysys_id'])->setLabel(___('Select saved card'))->loadOptions(
                    array_reduce($methods, function ($carry, $item){
                        $carry[$item['id']] = sprintf(
                            "%s .... %d exp. %02d/%04d",
                            strtoupper($item['card']['brand']),
                            $item['card']['last4'],
                            $item['card']['exp_month'],
                            $item['card']['exp_year']
                        );
                ...
                $form->addAdvRadio('payment_method', ['id' => 'paysys_id'])->setLabel(___('Add new card'))->loadOptions(
                    [0 => '']
                )->setValue(0);
                $form->addScript()->setScript(<<<CUT
jQuery(document).ready(function($) {
    $("input[name='payment_method']").change(function(){
        $("#{$id}-card-element").parent().parent().children(".am-element-title").toggle(!$("input[name='payment_method']").is(":checked") || (this.value == '0'));
        $("#{$id}-card-element").toggle(!$("input[name='payment_method']").is(":checked") || (this.value == '0'));
    }).change();
});                    
CUT
                );
...
jQuery('#{$id}-card-element').closest('form').on("amFormSubmit", function(event){
    ...
    event.callback = null; // wait for completion
        if(jQuery("input[name='payment_method']").is(":checked") && (jQuery("input[name='payment_method']:checked").val() != '0')){
            createPaymentIntent(jQuery("input[name='payment_method']:checked").val());
        }else{
        ...
        function createPaymentIntent(pm)
        {
                fetch('{$createUrl}', {
                    method: 'POST',
                    headers: {
...
class Am_Paysystem_Transaction_Stripe_Refund extends Am_Paysystem_Transaction_CreditCard
{
    ...
    public function __construct(Am_Paysystem_Abstract $plugin, Invoice $invoice, $charge_id, $amount = null)
    {
        ...
        $request->setAuth($plugin->getConfig('secret_key'), '');
        $request->setHeader('Stripe-Version', '2020-08-27');
        if ($this->amount > 0) {
