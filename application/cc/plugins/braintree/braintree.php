...
class Am_Paysystem_Braintree extends Am_Paysystem_CreditCard
{
    ...
    {
        if (in_array($key, ['merchant_id', 'public_key', 'private_key', 'merchant_account_id']))
    ...
    function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addSelect('hosted', 'id="hosted-select"', [
            'options'=> [
...
CUT1
    ...
    {
        if(!$this->getDi()->invoiceTable->findFirstBy([
            'user_id' => $user->pk(),
...
{
    function __construct(Am_Paysystem_CreditCard $plugin, $formType, $invoice)
    {
        $this->plugin = $plugin;
        $this->invoice = $invoice;
        $this->formType = $formType??self::PAYFORM;
        $this->payButtons = [
        ...
        Am_Form::__construct('cc', ['action' => \Braintree\TransparentRedirect::url()]);
    }
    ...
    function createFormRegular()
    {
        ...
        $name->addText($fn . 'credit_card__cardholder_name', ['size' => 30])
        ...
        $this->addText($fn . 'credit_card__number', ['autocomplete' => 'off', 'size' => 22, 'maxlength' => 22])
        ...
        $this->addPassword($fn . 'credit_card__cvv', ['autocomplete' => 'off', 'size' => 4, 'maxlength' => 4])
        ...
        $bname->addText($fn . 'credit_card__billing_address__first_name', ['size' => 15])
            ->addRule('required', ___('Please enter first name'))
            ->addRule('regex', ___('Please enter first name'), '|^[a-zA-Z_\' -]+$|');

        $bname->addText($fn . 'credit_card__billing_address__last_name', ['size' => 15])
...
jQuery(document).ready(function(){
    ...
    braintree.client.create({
        ...
        braintree.hostedFields.create({
            ...
            frm.on("amFormSubmit", function(event){
...
CUT
    ...
    function createFormDropIn()
    {
        ...
        $this->addStatic('_cc_info', ['class' => 'am-no-label'])
            ->setContent(<<<CONT
...
.hosted-field {
  ...
  font-size: 14px;
    ...
    function addClientToken(Am_Form_CreditCard_Braintree $form, User $user = null)
    {
        ...
        $clientToken = \Braintree\ClientToken::generate(['customerId' => $customerId]);
    ...
    function createForm()
    {
        ...
        if ($this->plugin->getConfig('hosted') == Am_Paysystem_Braintree::MODE_TRANSPARENT_REDIRECT)
            $form->addHidden('tr_data')->setValue(
                \Braintree\TransparentRedirect::createCustomerData([
    ...
    function createUpdateForm()
    {
        ...
        if ($this->plugin->getConfig('hosted') == Am_Paysystem_Braintree::MODE_TRANSPARENT_REDIRECT)
            $form->addHidden('tr_data')->setValue(
                \Braintree\TransparentRedirect::updateCreditCardData([
    ...
    function updateAction()
    {
        ...
        else if (($this->plugin->getConfig('hosted') != Am_Paysystem_Braintree::MODE_TRANSPARENT_REDIRECT) && $this->getParam('nonce'))
        {
            ...
            if($this->getParam('credit_card__billing_address__street_address'))
                $vars['creditCard']['billingAddress'] = [
