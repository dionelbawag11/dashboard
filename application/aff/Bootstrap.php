...
/**
 * @title Affiliate
 * @desc This module handles management of affiliate program
 */
class Bootstrap_Aff extends Am_Module
{
    ...
    /**
     * Event: Get Affiliate Link
     * use $event->setReturn() to set link
     * @param User $user
     */
    const AFF_GET_GENERAL_LINK = 'affGetGeneralLink';
    ...
    const ADMIN_PERM_APPROVE = 'aff-approve';
    ...
    function init()
    {
        ...
        $this->getDi()->userTable->customFields()->addCallback(['Am_Aff_PayoutMethod', 'static_addFields']);
    }
    ...
    function renderAlert()
    {
        if ($user_id = $this->getDi()->auth->getUserId()) {
            ...
            if ($user->is_affiliate > 0 && !in_array($user->aff_payout_type, $this->getConfig('payout_methods', []))) {
    ...
    {
        $e->addReturn(new Am_AdminDashboardWidget('aff-top-affiliate', ___('Top Affiliate'), [$this, 'renderWidgetTopAffiliate'], Am_AdminDashboardWidget::TARGET_ANY, [$this, 'createWidgetTopAffiliateConfigForm'], self::ADMIN_PERM_ID));
        $e->addReturn(new Am_AdminDashboardWidget('aff-last-user',  ___('Recent Affiliates'), [$this, 'renderWidgetLastUser'], Am_AdminDashboardWidget::TARGET_ANY, [$this, 'createWidgetNumForm'], self::ADMIN_PERM_ID));
        $e->addReturn(new Am_AdminDashboardWidget('aff-last-comm',  ___('Recent Affiliate Commissions'), [$this, 'renderWidgetLastComm'], Am_AdminDashboardWidget::TARGET_ANY, [$this, 'createWidgetNumForm'], self::ADMIN_PERM_ID));
        $e->addReturn(new Am_AdminDashboardWidget('aff-last-click', ___('Recent Affiliate Clicks'), [$this, 'renderWidgetLastClick'], Am_AdminDashboardWidget::TARGET_ANY, [$this, 'createWidgetNumForm'], self::ADMIN_PERM_ID));
    }

    public function renderWidgetTopAffiliate(Am_View $view, $config = null)
    {
        $intervals = is_null($config) ? [Am_Interval::PERIOD_THIS_WEEK_FROM_SUN] : (array)$config['interval'];
        $out = '';
        foreach ($intervals as $interval) {
            [$start, $stop] = $this->getDi()->interval->getStartStop($interval);
    ...
    function createWidgetTopAffiliateConfigForm()
    {
        ...
        $form->addSortableMagicSelect('interval', null, ['options' => $this->getDi()->interval->getOptions()])
            ->setLabel(___('Period'))
            ->setValue([Am_Interval::PERIOD_THIS_WEEK_FROM_SUN]);
        ...
        $form->addInteger('num', ['placeholder' => 10])
    ...
    {
        $list = $e->getList();
        $all = $list->gridMethods;

        $list->addDefinition('aff-payouts-details', function(Am_Di $di){ return new Am_ApiController_Table($di, $di->affPayoutDetailTable); },
            'Affiliate Payouts', $all);

        $list->addDefinition('aff-payouts', function(Am_Di $di){
            return (new Am_ApiController_Table($di, $di->affPayoutTable))
                ->addNested('aff-payout-details', true);
        }, 'Affiliate Payout Details', $all);
    ...
    {
        $event->addReturn([
            'method' => [$this->getDi()->affClickTable, 'clearOld'],
    ...
    function onAdminWarnings(Am_Event $event)
    {
        ...
        if(((empty($_SERVER['HTTPS']) || ($_SERVER['HTTPS'] != 'on'))) && ((empty($_SERVER['HTTP_X_FORWARDED_PROTO']) || ($_SERVER['HTTP_X_FORWARDED_PROTO'] != 'https'))))
            $event->addReturn(___('Please install SSL if you are using affiliate links with redirect to other domains'));
    }
    ...
    function onSetupEmailTemplateTypes(Am_Event $e)
    {
        ...
        $e->addReturn([
            ...
            'isAdmin' => true,
            'vars' => ['affiliate']
        ], 'aff.admin_registration_mail');
        $e->addReturn([
            'id' => 'aff.manually_approve_admin',
            'title' => 'Require Approval Notification to Admin',
            'mailPeriodic' => Am_Mail::REGULAR,
            'isAdmin' => true,
            'vars' => ['affiliate']
        ], 'aff.manually_approve_admin');
        $e->addReturn([
            'id' => 'aff.manually_approve',
            'title' => 'Require Approval Notification to Affiliate',
            'mailPeriodic' => Am_Mail::REGULAR,
            'vars' => ['affiliate']
        ], 'aff.manually_approve');
        $e->addReturn([
            'id' => 'aff.manually_approve_denied',
            'title' => 'Denied Notification to Affiliate',
            'mailPeriodic' => Am_Mail::REGULAR,
            'vars' => ['reason' => 'Denied Reason', 'affiliate']
        ], 'aff.manually_approve_denied');
    }
    ...
    {
        $e->addReturn([
            Am_Upload_Acl::IDENTITY_TYPE_ADMIN => [
    ...
    function onGetPermissionsList(Am_Event $e)
    {
        ...
        $e->addReturn("Affiliate: can approve new affiliates", self::ADMIN_PERM_APPROVE);
    }

    function onUserMenuItems(Am_Event $e)
    {
        $e->addReturn([$this, 'buildMenu'], 'aff');
    }
    ...
    {
        $menu = $event->getMenu();
        $await_cnt = $this->getDi()->db->selectCell("SELECT COUNT(*) FROM ?_data WHERE `table`='user' AND `key`='aff_await_approval';");
        $menu->addPage([
            'id' => 'affiliates',
            'uri' => 'javascript:;',
            'label' => ___('Affiliates'),
            'resource' => [self::ADMIN_PERM_ID, self::ADMIN_PERM_ID_BANNERS],
            'order' => 80,
            'pages' => array_merge([
                [
                ...
                ($this->getConfig('signup_type') != 2) ? [] : [
                    [
                        'id' => 'user-not-approved',
                        'controller' => 'admin-not-approved',
                        'module' => 'aff',
                        'action' => 'index',
                        'label' => ___('Not Approved') . ($await_cnt ? " (($await_cnt))" : ''),
                        'resource' => self::ADMIN_PERM_APPROVE
                    ]
                ],
            [
                [
    ...
    public function addPayoutInputs(HTML_QuickForm2_Container $fieldSet)
    {
        $el = $fieldSet->addSelect('aff_payout_type')
                ...
                ->loadOptions(array_merge(['' => ___('Not Selected')]));
...
');
    ...
    public function onGridCouponInitGrid(Am_Event_Grid $event)
    {
        $event->getGrid()->addField('aff_id', ___('Affiliate'))
            ->setRenderFunction([$this, 'renderAffiliate']);
    ...
    source: amUrl("/aff/admin/autocomplete/")
            ...
            ->setUpdateCallback(function($ds, $record, $fieldName, $v){
                ...
                $record->updateQuick('aff_id', $user ? $user->pk() : null);
            })
            ->getDecorator()->setInputTemplate(sprintf('<input type="text" placeholder="%s" />',
                ___('Type Username or E-Mail')));
        $event->getGrid()->addCallback(Am_Grid_ReadOnly::CB_RENDER_CONTENT, [$this, 'couponRenderContent']);
    }
    ...
    function onGridCouponBatchInitForm(Am_Event $event)
    {
        ...
        $affEl = $affGroup->addText('_aff', ['placeholder' => ___('Type Username or E-Mail')])
                ->setId('aff-affiliate');
...
CUT
        ...
        if (!empty($batch->aff_id)) {
            try {
                ...
                $affHtml = sprintf('<div><a href="%s">%s %s (%s)</a> <a href="javascript:;" title="%s" style="text-decoration:none; color:#ba2727" id="aff-unassign-affiliate">&#10005;</a></div>',
...
CUT
        ...
        if (!empty($input['_aff'])) {
            ...
            if ($aff) {
                ...
                if ($event->getGrid()->getRecord()->aff_id != $aff->pk()) {
                    ...
                    $aff_id = $this->getDi()->hook->filter($aff->pk(), self::AFF_BIND_AFFILIATE, [
    ...
    public function onGridUserInitForm(Am_Event_Grid $event)
    {
        ...
        $fieldSet = $event->getGrid()->getForm()
            ->addAdvFieldset(null, ['id' => 'affiliate'])
...
CUT
            ...
            switch ($match[1]) {
                ...
                case 'coupon':
                    $res = ___('Assigned by Coupon %s at %s',
                            '<a href="' . $this->getDi()->url('admin-coupons', ['_coupon_filter'=>$match[3]]) . '">' . $match[3] . '</a>',
    ...
    function getAffiliate(Invoice $invoice, InvoicePayment $payment = null)
    {
        ...
        try {
            if (!$aff_id && $coupon = $invoice->getCoupon()) { // try to find affiliate by coupon
                $aff_id = $coupon->aff_id ?: $coupon->getBatch()->aff_id;
            }
        ...
        if(empty($aff_id))
            switch($this->getModelType())
            {
                ...
                case self::MODEL_LAST_CLICK_WINS :
                    $aff_id = (int)$this->findAffId($aff_source);
                    break;
        ...
        $aff_id = $this->getDi()->hook->filter($aff_id, Bootstrap_Aff::AFF_FIND_AFFILIATE, [
            'invoice' => $invoice,
            'payment' => $payment,
        ]);
    ...
    function findAffId(& $aff_source = null)
    {
        ...
        if (defined('AM_CRON') && AM_CRON)
            return;

        $aff_id = !empty($_COOKIE[self::COOKIE_NAME]) ? $_COOKIE[self::COOKIE_NAME] : null;
        if ($aff_id) {
            $aff_id = preg_replace('#[^a-zA-Z0-9_=+/-]#', '', $aff_id);
        }
        $aff_source = null;
        //backwards compatibility of affiliate cookies
...
//        }
    ...
    function onUserBeforeInsert(Am_Event_UserBeforeInsert $event)
    {
        ...
        if (intval($aff_id) > 0) {
            $this->setAffiliate($event->getUser(), intval($aff_id), $aff_source);
        }
        // remember for usage in onUserAfterInsert
        $this->last_aff_id = $event->getUser()->aff_id == intval($aff_id) ? $aff_id : null;

        if (empty($event->getUser()->is_affiliate))
            $event->getUser()->is_affiliate = $this->getDi()->config->get('aff.signup_type') == 1 ? 1 : 0;
    }

    function setAffiliate($user, $aff_id, $aff_source = null)
    {
        $aff_id = $this->getDi()->hook->filter($aff_id, self::AFF_BIND_AFFILIATE, [
            'user' => $user
        ]);
        ...
        if ($aff_source)
            $user->data()->set('aff-source', $aff_source);
    }
    ...
    function onUserAfterInsert(Am_Event_UserAfterInsert $event)
    {
        ...
        if ($this->last_aff_id && preg_match('/^(\d+)-(\d+)-(-?\d+)$/', $this->last_aff_id, $regs)) {
    ...
    {
        foreach (['?_aff_click', '?_aff_commission', '?_aff_lead'] as $table) {
    ...
    {
        if($e->getUser()->is_affiliate && !$e->getOldUser()->is_affiliate && ($this->getConfig('signup_type') == 2)) {
            $u = $e->getUser();
            $u->data()->set('aff_await_approval', null);
            $u->save();
    ...
    function onAffCommissionAfterInsert(Am_Event $event)
    {
        ...
        if ($this->getConfig('notify_payout_empty')) {
            /** @var User $aff */
            $aff = $event->getAff();
            if (in_array($aff->aff_payout_type, $this->getConfig('payout_methods', [])) ||
    ...
    function setCookie(User $aff, /* AffBanner */ $banner = null, $aff_click_id = null)
    {
        ...
        Am_Cookie::setSamesiteNone(self::COOKIE_NAME, $val, $tm, '/', $_SERVER['HTTP_HOST']);
    }
    ...
    function setKeywordCookie(User $aff, $keyword=null)
    {
        $tm = $this->getDi()->time + $this->getDi()->config->get('aff.cookie_lifetime', 30) * 3600 * 24;
        if(!is_null($keyword))
            Am_Cookie::setSamesiteNone(sprintf("%s-%s", self::COOKIE_NAME, md5($aff->login)), base64_encode($keyword), $tm, '/', $_SERVER['HTTP_HOST']);
    }
    ...
    function onDaily(Am_Event $e)
    {
        ...
        foreach ($payout_day as $delay) {
            ...
            [$count, $unit] = preg_split('/(\D)/', $delay, 2, PREG_SPLIT_DELIM_CAPTURE);
            switch ($unit) {
                case 'd':
                    if($count == 31)
                    {
                        if(date('j', amstrtotime($e->getDatetime())) != date('t', amstrtotime($e->getDatetime())))
                            continue 2;
                        break;
                    }
 ...
 AND v.commission_id IS NOT NULL");
    ...
    public function onMailTemplateBeforeParse(Am_Event $event)
    {
        ...
        foreach (['user', 'affiliate'] as $prefix) {
    ...
    {
        $event->getBlocks()->add('member/main/top', new Am_Block_Base(null, 'aff-member-payout-empty', null, [$this, 'renderAlert']));
        $event->getBlocks()->add('aff/top', new Am_Block_Base(null, 'aff-aff-payout-empty', null, [$this, 'renderAlert']));

        $event->getBlocks()->add('aff/links/middle', new Am_Widget_AffIntro);
        $event->getBlocks()->add('aff/links/middle', new Am_Widget_AffGeneralLink);
        ...
        if (($admin = $this->getDi()->authAdmin->getUser()) && $admin->hasPermission(self::ADMIN_PERM_ID)) {
            $event->getBlocks()->add('admin/user/invoice/details',new Am_Block_Base(null, 'aff-user-invoice-details', null, [$this, '_renderInvoiceCommissions']));
    ...
    function getGeneralAffLink(User $user)
    {
        return $this->getDi()->hook->filter(
            $this->getDi()->surl('aff/go/'.urlencode($user->login), false),
            self::AFF_GET_GENERAL_LINK,
            ['user' => $user]
        );
    }
    ...
    function getClickJs()
    {
        $root_url = json_encode($this->getDi()->rurl('aff/click-js/', false));
        $root_surl = json_encode($this->getDi()->surl('aff/click-js/', false));
...
EOT;
    ...
    protected static function setUpPayoutsIfNone()
    {
        ...
        if (null === $config->get('aff.payout_methods', null)) {
            $config->saveValue('aff.payout_methods', ['check']);
            $config->set('aff.payout_methods', ['check']);
        }
    }

    /**
     * @return string Affiliate Program Model (default or last_click_wins)
     */
     ...
     * Track clicks
     */
    function getTrackingLink($aff, $banner_id = null, $escape = true, $params = [])
    {
        if (is_object($aff)) {
            $aff = $aff->login;
        }
    ...
    function getCustomTrackingLink($aff, $url, $escape = true)
    {
        if (is_object($aff))
            $aff = $aff->login;
        return $this->getDi()->url('aff/go/'.urlencode($aff), ['cr'=>base64_encode($url)],$escape,2);
    }
    ...
    function getBannerHtml($aff, $banner_id)
    {
        include_once AM_APPLICATION_PATH . '/aff/controllers/BannerController.php';
        if (!($banner = $this->getDi()->affBannerTable->load($banner_id, false))) return;
        $bannerRenderer = Am_BannerRenderer::create($banner, $aff);
        return $bannerRenderer->getCodeHtml();
    }

    function logClick()
    {
        $this->aff = $this->findAff();
        $this->link = $this->getDi()->hook->filter($this->findUrl(), Am_Event::GET_AFF_REDIRECT_LINK, ['aff' => $this->aff]);
        /// log click
        if ($this->aff) {
    ...
    function findAff()
    {
        $id = preg_replace("/[^. @a-zA-Z0-9'_-]/", '', $ref = $this->getDi()->request->getParam('r', $this->getDi()->request->getParam('ref')));
        $aff = null;
        ...
        return $this->getDi()->hook->filter($aff, self::AFF_FIND_AFF_FROM_URL, ['aff_id' => $id, 'ref' => $ref]);
    }
