...
abstract class Am_Aff_PayoutMethod
{
    ...
    static function getEnabled()
    {
        if (!self::$enabled){
            ...
            foreach (Am_Di::getInstance()->config->get('aff.payout_methods', []) as $methodName) {
...
class Am_Aff_PayoutMethod_Paxum extends Am_Aff_PayoutMethod
{
    public function getTitle()
    {
        return ___("Paxum");
    ...
    public function export(AffPayout $payout, Am_Query $details, Am_Mvc_Response $response)
    {
        ...
        $this->sendCsv("paxum-commission-" . $payout->payout_id . ".csv", $rows, $response);
    }

    public function addFields(Am_CustomFieldsManager $m)
    {
        $m->add(new Am_CustomFieldText('aff_paxum_email', ___('Affiliate Payout - Paxum E-Mail address'), ___('for affiliate commission payouts')))->size = 40;
    }
}

class Am_Aff_PayoutMethod_Cashapp extends Am_Aff_PayoutMethod
{
    public function getTitle()
    {
        return ___("Cash App");
    ...
    public function export(AffPayout $payout, Am_Query $details, Am_Mvc_Response $response)
    {
        ...
        $this->sendCsv("cashapp-commission-" . $payout->payout_id . ".csv", $rows, $response);
    }

    public function addFields(Am_CustomFieldsManager $m)
    {
        $m->add(new Am_CustomFieldText('aff_cashapp_id', ___('Affiliate Payout - Cash App Username'), ___('for affiliate commission payouts')))->size = 40;
    }
}

class Am_Aff_PayoutMethod_Venmo extends Am_Aff_PayoutMethod
{
    public function getTitle()
    {
        return ___("Venmo");
    ...
    public function export(AffPayout $payout, Am_Query $details, Am_Mvc_Response $response)
    {
        ...
        while ($d = $payout->getDi()->db->fetchRow($q)) {
            ...
            $aff = $d->getAff();
            $rows[] = [
                preg_replace('/\D/', '', $aff->data()->get('aff_venmo_phone')),
                moneyRound($d->amount),
                Am_Currency::getDefault(),
                $aff->user_id,
                "Affiliate commission to " . amDate($payout->thresehold_date),
        ...
        $this->sendCsv("venmo-commission-" . $payout->payout_id . ".csv", $rows, $response);
    }

    public function addFields(Am_CustomFieldsManager $m)
    {
        $m->add(new Am_CustomFieldText('aff_venmo_phone',
            ___('Affiliate Payout - Domestic mobile telephone number'),
            ___('for affiliate commission payouts')))->size = 40;
    }
}
...
class Am_Aff_PayoutMethod_Coinpayments extends Am_Aff_PayoutMethod
{
    public function getTitle()
    {
        return ___("Coin Payments");
    }
    ...
    public function export(AffPayout $payout, Am_Query $details, Am_Mvc_Response $response)
    {
        ...
        while ($d = $payout->getDi()->db->fetchRow($q)) {
            ...
            $aff = $d->getAff();
            $rows[] = [
                $aff->data()->get('aff_coinpayments_wallet'),
                $aff->data()->get('aff_coinpayments_currency'),
                moneyRound($d->amount),
                Am_Currency::getDefault(),
                $aff->user_id,
                "Affiliate commission to " . amDate($payout->thresehold_date),
        ...
        $this->sendCsv("coinpayments-commission-" . $payout->payout_id . ".csv", $rows, $response);
    }

    public function addFields(Am_CustomFieldsManager $m)
    {
        $_ = ['USCD', 'Ether', 'Litecoin', 'Bitcoin Cash'];

        $m->add(new Am_CustomFieldText(
            'aff_coinpayments_wallet',
            ___('Affiliate Payout - Wallet'),
            ___('for affiliate commission payouts'))
        )->size = 40;
        $m->add(new Am_CustomFieldSelect(
            'aff_coinpayments_currency',
            ___('Affiliate Payout - Crypto Currency'),
            ___('for affiliate commission payouts'),
            null,
            ['options' => array_combine($_, $_)])
        );
    }
}
...
class Am_Aff_PayoutMethod_Directdeposit extends Am_Aff_PayoutMethod
{
    ...
    public function addFields(Am_CustomFieldsManager $m)
    {
        $m->add(new Am_CustomFieldSelect('aff_directdeposit_account_type', ___('Affiliate Direct Deposit - Account Type'), null, null, [
            'options' => [
                'checking' => 'Checking',
                'savings' => 'Savings'
            ]
...
class Am_Aff_PayoutMethod_Upi extends Am_Aff_PayoutMethod
{
    public function getTitle()
    {
        return ___("UPI");
    ...
    public function export(AffPayout $payout, Am_Query $details, Am_Mvc_Response $response)
    {
        ...
        $this->sendCsv("upi-commission-" . $payout->payout_id . ".csv", $rows, $response);
    }

    public function addFields(Am_CustomFieldsManager $m)
    {
        $m->add(new Am_CustomFieldText('aff_upi_address', ___('Affiliate Payout - UPI address'), ___('for affiliate commission payouts')))->size = 40;
    }
}