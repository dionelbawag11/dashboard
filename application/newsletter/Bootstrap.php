...
/**
 * @title Newsletters
 * @desc Sending and receiving e-mail messages (newsletters) to customers
 */
class Bootstrap_Newsletter extends Am_Module
{
    const NEWSLETTER_SIGNUP_DATA = 'newsletter_signup_data';
    /**
     * Event: called when user newsletter subscription removed
     *
     * @param User user
     * @param NewsletterList $list
     */
    const NEWSLETTER_LIST_SUBSCRIPTION_REMOVED = 'newsletterListSubscriptionRemoved';

    protected $isEnabledByAdminPlugins = false;

    function init()
    {
        $this->getDi()->plugins_newsletter = new Am_Plugins_Newsletter($this->getDi(),
            'newsletter', dirname(__FILE__) . '/plugins',
            'Am_Newsletter_Plugin_%s', '%s.%s'
        );
        if (AM_PHAR)
        {
            $this->getDi()->plugins_newsletter->setPharTemplate($this->getDi()->root_dir . "/amember-newsletter-%s.phar",
                ['application/newsletter/plugins/%s.php', 'application/newsletter/plugins/%s/%1$s.php']);

            $this->getDi()->plugins_newsletter->addPath($this->getDi()->root_dir . '/application/newsletter/plugins');
        }
        ...
        $this->getDi()->router->addRoute('newsletter-plugins', new Am_Mvc_Router_Route(
            'newsletter/:plugin_id/:action',
            [
                'module' => 'default',
                'controller' => 'direct',
                'action' => 'index',
        ...
        $this->getDi()->router->addRoute('newsletter-ajax', new Am_Mvc_Router_Route(
    ...
    function onGridUserInitForm(Am_Event_Grid $event)
    {
        ...
        $el = $form->addMagicSelect('_newsletter', ['class' => 'am-combobox'])->setLabel(___('Newsletter Subscriptions'));
    ...
    {
        $e->addReturn([
            'id' => 'newsletter.list_subscription_removed',
            'title' => 'Newsletter List Removed notification to user',
            'mailPeriodic' => Am_Mail::USER_REQUESTED,
            'vars' => [
                'user',
                'list' => ___('Newsletter list'),
            ],
        ], 'newsletter.list_subscription_removed');
    }

    function onNewsletterListSubscriptionRemoved(Am_Event $event)
    {
        $user = $event->getUser();
        if($user->unsubscribed)
            return;
        if ($this->getConfig('list_subscription_removed') && ($et = Am_Mail_Template::load('newsletter.list_subscription_removed')))
        {
                $et->setUser($user)
