...
CUT
        ...
        $form->addSelect("sra")
            ...
            ->loadOptions([
                '2' => 'Reattempt failed recurring payments before canceling',
                '1' => 'Do not reattempt failed recurring payments'
            ]);
        $form->addAdvCheckbox('pdt', ['id'=>'pdt-enabled'])->setLabel("Enable PDT support\n"
        ...
        $form->addText('pdt_token', ['class'=>'am-el-wide', 'id'=>'pdt-token'])->setLabel('PDT Identity Token');
    ...
    public function isNotAcceptableForInvoice(Invoice $invoice)
    {
        ...
        if ($invoice->rebill_times != IProduct::RECURRING_REBILLS && $invoice->rebill_times > 52)
            return ['PayPal can not handle subscription terms with number of rebills more than 52'];
    }
    ...
    function thanksAction($request, $response, array $invokeArgs)
    {
        if(empty($request->get('tx')))
            $response->setRedirect($this->getDi()->surl("thanks"));
        else
    ...
    public function getAdminCancelUrl(Invoice $invoice)
    {
        ...
        return 'https://' . $this->domain . '?' . http_build_query([
            'cmd' => '_subscr-find',
...
class Am_Paysystem_Paypal_Transaction extends Am_Paysystem_Transaction_Paypal
{
    protected $_autoCreateMap = [
        ...
        'invoice_external_id' => ['parent_txn_id', 'old_subscr_id', 'subscr_id', 'txn_id'],
    ];
    ...
    public function processValidated()
    {
        ...
        switch($this->request->payment_status){
            case 'Reversed':
                if ($originalInvoicePayment  = Am_Di::getInstance()->invoicePaymentTable->findFirstBy([
                    'receipt_id' => $this->request->parent_txn_id,
                    'invoice_id' => $this->invoice->pk()
                ])) {
                    Am_Di::getInstance()->accessTable->deleteBy([
    ...
    public function validateTerms()
    {
        ...
        if (in_array($this->txn_type, [self::TXN_CART, self::TXN_SUBSCR_PAYMENT, self::TXN_WEB_ACCEPT]))
        {
    ...
    public function autoCreateGetProducts()
    {
        ...
        if($billing_plan) return [$billing_plan->getProduct()];
    }
}
