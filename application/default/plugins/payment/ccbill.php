...
class Am_Paysystem_Ccbill extends Am_Paysystem_Abstract
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $fs = $form->addFieldset("advanced")->setLabel(___('Advanced Functionality'));
        $fs->addHTML()->setHTML(___('This functionality will work only if you set up to access the CCBill Data Link Extract System.'));
        $fs->addCheckbox('cancel_discounts')->setLabel(___('Enable Cancellation Discounts
        ...
        $fs->addTextarea('cancel_discount_offer', ['class' => 'am-el-wide', 'rows' => 5])->setLabel(___('Offer that will be displayed for user on cancellation
        You can use &#37;amount&#37; placeholder in text(this is amount of discount)
        '));

        $fs->addAdvCheckbox('attempt-to-charge')->setLabel(___('Attempt to charge existing customer without redirect
        ...
        jQuery(document).ready(function(){
            jQuery("input[name$='cancel_discounts']").on('change', function(){
                jQuery("textarea[name$='cancel_discount_offer']").closest(".am-row").toggle(jQuery(this).is(":checked"))
            }).change();
        });
CUT
);
    $form->setDefault('cancel_discount_offer', $this->discountOffer());

    }
    function discountOffer(){
        return $this->getConfig('cancel-discount-offer',  ___("We whould like to offer you a $%amount% discount if you decide to stay with our services!"));
    }

    function supportsChangeRebillDate()
    {
        return true;
    }

    function changeRebillDate(Invoice $invoice, DateTime $rebill_date)
    {
        $oldDate = new DateTime($invoice->rebill_date);
        $daysDiff = $rebill_date->diff($oldDate)->days;

        if($daysDiff<1)
        {
            throw new Am_Exception_InputError("Can't set rebill date in the past");
        }
        $this->smsApiRequest($invoice, function($subaccountId, $subscriptionId) use($daysDiff){
            return [
                'clientAccnum' => $this->getConfig('account'),
                'clientSubacc' => $subaccountId,
                'usingSubacc' => $subaccountId,
                'returnXML' => 1,
        ...
        parent::changeRebillDate($invoice, $rebill_date);
    }
    ...
    function tryToCharge(Invoice $invoice, Am_Paysystem_Result $result)
    {
        $oldInvoiceId = $this->getDi()->db->selectCell(
            "select invoice_id from ?_invoice where user_id=? and paysys_id=? and status>1 order by invoice_id desc limit 1",
            $invoice->getUser()->pk(),
            $this->getId()
        );
        ...
        $oldInvoice = $this->getDi()->invoiceTable->load($oldInvoiceId);
        ...
        $payments  = $oldInvoice->getPaymentRecords();
...
//    	      'clientSubacc' =>$this->getConfig('subaccount_id'),
            ...
            'initialPrice' => $invoice->first_total,
            'initialPeriod' => $this->getDays($invoice->first_period),
            'returnXML' => 1,
            'username' => $this->getConfig('datalink_user'),
            'password' => $this->getConfig('datalink_pass'),
        ];
        ...
        {
            $vars['recurringPrice'] = $invoice->second_total;
            $vars['recurringPeriod'] = $this->getDays($invoice->second_period);
            $vars['rebills'] = $invoice->rebill_times == IProduct::RECURRING_REBILLS ? 99 : $invoice->rebill_times;
        } else {
        ...
        try{
            ...
            if((string)$xml->approved != "1")
                throw new Am_Exception_InternalError('ccBill Subscription Management error: Incorrect response received while attempting to charge  subscription!');

        }catch(Exception $ex){
            $this->getDi()->logger->error($ex->getMessage());
            return false;
        }
    ...
    {
        if($this->getConfig('attempt-to-charge')){
            if($this->tryToCharge($invoice, $result))
            {
                return $result;
            }
        }
    ...
    {
        $this->smsApiRequest($invoice, function($subaccountId, $subscriptionId){
            return [
                'clientAccnum' => $this->getConfig('account'),
                'clientSubacc' => $subaccountId,
                'usingSubacc' => $subaccountId,
...
CUT;
    ...
    function onHourly()
    {
        ...
        $subaccounts = $this->getDi()->db->selectCol("
            SELECT `value` 
            FROM ?_data 
            WHERE `key` = 'ccbill_subaccount_id'
            AND `table` = 'billing_plan'");
        $subaccounts[] = $this->getConfig('subaccount_id');
        $subaccounts = array_filter(array_unique($subaccounts));

        foreach($subaccounts as $subaccount) {
            $last_run = $this->getDi()->store->get(self::CCBILL_LAST_RUN."-".$subaccount);
            if(!$last_run){
...
// "REBILL","545455","0001","0312112601000035867","2012-05-21","0112142105000024293","6.10"
            ...
            $this->getDi()->store->set(self::CCBILL_LAST_RUN.'-'.$subaccount, $now_run);
        }
    }
     ...
     * @param callable $getParams => function($subaccountId, $subscriptionId): Array
     * @return void
     * @throws Am_Exception_InputError
     * @throws Am_Exception_InternalError
     * @throws HTTP_Request2_Exception
     */
    function smsApiRequest(Invoice $invoice, callable $getParams)
    {
        if (!$this->getConfig('datalink_user') || !$this->getConfig('datalink_pass'))
        {
            throw new Am_Exception_InputError("ccBill plugin error: Datalink is not configured!");
        }
        $payments  = $invoice->getPaymentRecords();
        ...
        if((string)$xml != "1")
            throw new Am_Exception_InternalError('ccBill Subscription Management error: Incorrect response!');

        return json_decode(json_encode($xml), true);

    }

    function getDiscount(Invoice $invoice)
    {
      try{
            $ret = $this->smsApiRequest($invoice, function($subaccountId, $subscriptionId){
                return [
                    'clientAccnum' => $this->getConfig('account'),
                    'clientSubacc' => $subaccountId,
                    'usingSubacc' => $subaccountId,
                    'returnXML' => 1,
                    'action' => 'viewDiscountInfo',
                    'subscriptionId' => $subscriptionId,
                    'username' => $this->getConfig('datalink_user'),
                    'password' => $this->getConfig('datalink_pass')
                ];
        ...
        if(!empty($ret['discountInfo']))
        {
            $data = empty($ret['discouintInfo'][0])? [$ret['discountInfo']] : $ret['discountInfo'];
            foreach($data as $discount){
                if($discount['type'] == 'CANCEL')
                    return $discount;
            }
        }
        return false;
    ...
    function applyDiscount(Invoice  $invoice)
    {
        $this->smsApiRequest($invoice, function($subaccountId, $subscriptionId){
            return [
                'clientAccnum' => $this->getConfig('account'),
    ...
    public function directAction($request, $response, array $invokeArgs)
    {
        ...
        {
            if($id = $request->getFiltered('customVar1'))
            {
                if($invoice = $this->getDi()->invoiceTable->findFirstBy(['public_id' => $id]))
                    return $response->setRedirect($this->getDi()->surl("cancel", ['id'=>$invoice->getSecureId("CANCEL")], false));
            }
            elseif($this->getConfig('cancel_discounts')) {

                $invoice = $this->getDi()->invoiceTable->findBySecureId($request->getParam('id'), 'STOP' . $this->getId());
                if ($invoice && (($discount = $this->getDiscount($invoice)) !== false)) {
                    if ($request->getParam('r')) {
                        if ($request->getParam('r') == 'yes') {
                            $this->applyDiscount($invoice);
                            return $response->setRedirect($this->getDi()->surl('member'));
                        }
                    } else {
                        $view = $this->getDi()->view;
                        $view->title = ___('Cancel Subscripton');
                        $offer = new Am_SimpleTemplate();
                        $offer->assign('amount', $discount['amount']);
                        $txt = $offer->render($this->discountOffer());
                        $yesUrl = $this->getPluginUrl('cancel', ['id' => $request->getParam('id'), 'r' => 'yes']);
                        $noUrl = $this->getPluginUrl('cancel', ['id' => $request->getParam('id'), 'r' => 'no']);
                        $view->content = <<<CONTENT
<div class="am-ccbill-discount-offer">
{$txt}
</div>
<div style="display:flex; justify-content: center; width:100%; gap: 2em; padding-top: 2em; align-items: center">
    <div><button onClick="document.location.href='{$yesUrl}'">YES, I would like to apply discount!</button></div>
    <div><a href="{$noUrl}">NO, Please cancel subscription!</a></div>
</div>
CONTENT;
