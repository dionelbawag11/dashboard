...
class Am_Paysystem_PaypalExpress extends Am_Paysystem_Abstract
{
    ...
    public function _initSetupForm(Am_Form_Setup $form)
    {
        ...
        $form->addAdvCheckbox("dont_verify")
            ...
            websites. It breaks functionality of the PayPal payment integration plugin,
...
CUT;
    ...
    public function expressCheckoutAction($request, $response, array $invokeArgs)
    {
        ...
        if ($invoice->rebill_times)
        {
            ...
            $vars = $apireq->sendRequest($log);
            if (!in_array($vars['ACK'], ['Success', 'SuccessWithWarning']))
            {
...
class Am_Paysystem_Transaction_PayPalExpress_DoExpressCheckout extends Am_Paysystem_Transaction_Abstract
{
    ...
    {
        if (!in_array($this->vars['ACK'], ['Success', 'SuccessWithWarning'])) {
        ...
        if (!in_array($this->vars['PAYMENTINFO_0_PAYMENTSTATUS'], ['Completed', 'Processed'])) {
