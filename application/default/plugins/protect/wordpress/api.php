...
class WordpressAPI
{
    ...
    protected $_options = [];
    protected $_role_key = "";
    protected $_amember_roles = ['amember_active'=> 'aMember Active', 'amember_expired'=> 'aMember Expired'];
    ...
    {
        $this->is_multisite = $plugin->haveNetworkSupport();
        $this->_plugin = $plugin;
        $this->_role_key = $this->_plugin->getConfig('prefix')."user_roles";
        $this->COOKIEHASH = ($siteurl = $this->get_siteurl()) ? md5($siteurl) : '';
    ...
    function get_siteurl()
    {
        if ($this->is_multisite) {
            return $this->getDb()->selectCell("SELECT meta_value FROM ?_sitemeta WHERE meta_key='siteurl' AND site_id=1");
        } else {
            return $this->wp_url_filter(defined('WP_SITEURL') ? WP_SITEURL : $this->get_option('siteurl'));
        }
    }
    ...
    function get_roles()
    {
        ...
        foreach($this->_amember_roles as $r=>$n){
            if(!array_key_exists($r, $roles)){
                $this->add_role($r, $n, ['read'=>1, 'level_0'=>1]);
            }
        }
        return $roles;
    }
    ...
    {
        $roles = $this->get_option($this->_role_key);
        $roles[$role] = ['name'=>$name, 'capabilities' =>$caps];
    ...
    function wp_set_auth_cookie($user_id, $remember = false, $secure = '', Am_Record $user=null)
    {
        ...
        $_COOKIE[$this->LOGGED_IN_COOKIE] = $logged_in_cookie;
    }
    ...
    function destroySessionTokens($user_id)
    {
        $this->update_user_meta($user_id, 'session_tokens', null);
    }

    function verifySessionToken($user_id, $token)
    {
        $sessions = $this->getSessions($user_id);
        $verifier  = hash('sha256', $token);
    ...
    function wp_validate_auth_cookie($cookie = '', $scheme = '')
    {
        ...
        if (!hash_equals($cookie['hmac'], $hash)) {
            return false;
        }

        switch($this->getVersion()) {
            case 4 :
                if (!$this->verifySessionToken($user['ID'], $cookie['token'])) {
                    return false;
                }
                return $user['ID'];
            default:
