...
/**
 * @title Helpdesk
 * @desc Customer support helpdesk
 */
class Bootstrap_Helpdesk extends Am_Module
{
    ...
    const EVENT_TICKET_AFTER_UPDATE = 'helpdeskTicketAfterUpdate';
    const EVENT_TICKET_AFTER_DELETE = 'helpdeskTicketAfterDelete';

    const EVENT_MESSAGE_BEFORE_INSERT = 'helpdeskMessageBeforeInsert';
    const EVENT_MESSAGE_AFTER_INSERT = 'helpdeskMessageAfterInsert';
    ...
    function renderNotification()
    {
        if ($user_id = $this->getDi()->auth->getUserId()) {
            ...
            if ($cnt) {
                return '<div class="am-info am-info-helpdesk">' . ___('You have %s%d ticket(s)%s that require your attention',
                    sprintf('<a href="%s">', $this->getDi()->url('helpdesk', ['_user_has_new'=>1])), $cnt, '</a>') .
                '</div>';
            }
        }
    }
    ...
    function onSetupEmailTemplateTypes(Am_Event $e)
    {
        ...
        $from = $this->getConfig('email_from') ?
            [$this->getConfig('email_from'), $this->getConfig('email_name')] :
            null;
        ...
        $e->addReturn([
            ...
            'vars' => $ticket + ['url' => 'Url of Page with Message', 'user'],
        ...
        $e->addReturn([
            ...
            'mailPeriodic' => Am_Mail::USER_REQUESTED,
            'vars' => $ticket + ['url' => 'Url of Page with Ticket', 'user'],
        ], 'helpdesk.new_ticket');
        $e->addReturn([
        ...
        $e->addReturn([
            ...
            'mailPeriodic' => Am_Mail::USER_REQUESTED,
            'vars' => ['user'] + $ticket + ['url' => 'Url of Page with Ticket'],
    ...
    {
        $event->addReturn(new Am_AdminDashboardWidget('helpdesk-messages', ___('Last Messages in Helpdesk'), [$this, 'renderWidget'], Am_AdminDashboardWidget::TARGET_ANY, [$this, 'createWidgetConfigForm'], self::ADMIN_PERM_ID));
    }
    ...
    {
        $e->addReturn([
            'method' => [$this->getDi()->helpdeskTicketTable, 'clearOld'],
    ...
    {
        $cntMy = $this->getDi()->helpdeskTicketTable->countBy([
        ...
        $event->getMenu()->addPage([
            ...
            'uri' => '#',
            'id' => 'helpdesk',
            'resource' => [self::ADMIN_PERM_ID, self::ADMIN_PERM_FAQ, self::ADMIN_PERM_CATEGORY],
            'order' => 90,
            'pages' => [
                [
                    'label' => ___('Dashboard') . ($cntDashboard ? " (($cntDashboard))" : ''),
                    'uri' => $this->getDi()->url('helpdesk/admin', [
                        '_dashboard_filter_s' => [
                ...
                [
                    ...
                    'action' => 'p',
                    'module' => 'helpdesk',
                    'id' => 'helpdesk-snippets',
                    'resource' => self::ADMIN_PERM_CATEGORY,
                    'params' => [
                        'view' => 'displaysnippets',
                        'f' => 'menu'
                    ],
                    'order' => 60,
                ]
    ...
    {
        $e->addReturn([$this, 'buildMenu'], 'helpdesk');
    }
    ...
    {
        $e->getBlocks()->add('member/main/top', new Am_Block_Base(null, 'helpdesk-notification', null, [$this, 'renderNotification']));
    }
    ...
    function onDbUpgrade(Am_Event $e)
    {
            ...
            if (!$this->getDi()->db->selectCell("SELECT COUNT(*) FROM ?_resource_access WHERE resource_type=?", HelpdeskCategory::ACCESS_TYPE)) { //idempotent action
                ...
                foreach($this->getDi()->helpdeskCategoryTable->findBy() as $cat) {
                        $this->getDi()->resourceAccessTable
                            ->setAccess($cat->pk(), HelpdeskCategory::ACCESS_TYPE, [
        ...
        if (version_compare($e->getVersion(), '6.1.7') <= 0) {
            $this->getDi()->db->query("SET @i = 0; UPDATE ?_helpdesk_snippet SET sort_order=(@i:=@i+1) ORDER BY sort_order;");
        }
        $cats = [];
        foreach ($this->getDi()->helpdeskFaqTable->findBy([]) as $faq) {
            if ($faq->category) {
                if (!isset($cats[$faq->category])) {
                    $cat = $this->getDi()->helpdeskFaqCategoryRecord;
                    $cat->title = $faq->category;
                    $cat->save();
                    $cats[$faq->category] = $cat->pk();
                }

                $faq->faq_category_id = $cats[$faq->category];
                $faq->category = null;
                $faq->save();
            }
        }
